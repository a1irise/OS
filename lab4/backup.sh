...
then
	...
	do
		...
		then
			cp -r $source/$file $prev_backup
			echo $file >> new_files
			new_flag=1
		else
			old_size=$(wc -c $prev_backup/$file | awk '{print $1}')
	...
	[[ $new_flag -eq 1 || $modified_flag -eq 1 ]] && echo "Directory $prev_backup has been modified on $date" >> $backup_report
